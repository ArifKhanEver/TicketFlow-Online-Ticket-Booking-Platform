...
// ১. রিয়েল-টাইম রানিং কাউন্টডাউন ক্যাপসুল ব্যাজ (image_7adba4.png এর মতো ডিজাইন)
const CountdownTimer = ({ departureTime, status }) => {
  ...
  useEffect(() => {
    if (status === "rejected") {
      setTimeLeft(""); // শর্ত: রিজেক্টেড হলে কাউন্টডাউন রিমুভ হবে
      return;
    }
...
export default function MyBookedTicketsPage() {
  // ডামি ডাটা সেট (টাইমলাইন ভেক্টরের জন্য journeyDuration যুক্ত করা হয়েছে)
  const [bookings, setBookings] = useState([
    ...
    {
      _id: "b2",
      title: "Green Line Paribahan - Sleeper Class",
      image: "https://images.unsplash.com/photo-1570125909232-eb263c188f7e?q=80&w=600",
      bookingQuantity: 1,
      unitPrice: 1800,
      from: "Dhaka",
      to: "Cox's Bazar",
      departureDateTime: "2026-08-01T08:30:00",
      journeyDuration: 10,
      status: "accepted"
    },
    {
      _id: "b3",
      title: "Subarna Express - Snigdha AC",
      image: "https://images.unsplash.com/photo-1474487548417-781cb71495f3?q=80&w=600",
      bookingQuantity: 3,
      unitPrice: 850,
      from: "Dhaka",
      to: "Chattogram",
      departureDateTime: "2026-07-20T16:30:00",
      journeyDuration: 5,
      status: "paid"
    },
    {
      _id: "b4",
      title: "US-Bangla Airlines - Boeing 737",
      image: "https://images.unsplash.com/photo-1436491865332-7a61a109cc05?q=80&w=600",
      bookingQuantity: 1,
      unitPrice: 4500,
      from: "Dhaka",
      to: "Sylhet",
      departureDateTime: "2026-06-10T11:00:00",
      journeyDuration: 1,
      status: "rejected"
    }
  ]);
      ...
      <div className="flex flex-col gap-6 w-full">
        <AnimatePresence>
          {bookings.map((booking, idx) => {
            ...
            // অ্যারাইভাল টাইম মেথড ক্যালকুলেশন
            const depDate = new Date(booking.departureDateTime);
            const arrDate = new Date(depDate.getTime() + (booking.journeyDuration || 0) * 60 * 60 * 1000);
            ...
            return (
              ...
              >
                {/* ফিক্সড: image_7adba4.png এর অনুরূপ হরিজন্টাল প্রিমিয়াম লেআউট স্ট্রাকচার */}
                <Card className="bg-white dark:bg-[#111113] border border-zinc-200 dark:border-zinc-800 shadow-sm hover:shadow-xl hover:border-[#039855]/40 transition-all duration-300 flex flex-col md:flex-row items-stretch rounded-[28px] overflow-hidden w-full">
                  ...
                  <div className="w-full md:w-64 h-48 md:h-auto relative shrink-0 bg-zinc-100 dark:bg-zinc-900">
                    ...
                    <span className="absolute top-3 left-3 text-[10px] font-black tracking-widest uppercase px-2.5 py-1 bg-black/60 backdrop-blur-md text-white rounded-lg shadow-sm flex items-center gap-1">
                      <FiTag size={11} className="text-[#039855]" /> Booked
                    </span>
                  </div>
                  ...
                  <div className="flex-1 p-6 flex flex-col justify-between gap-6">
                    ...
                    {/* Row 1: Title, Reservation Status, and Countdown Badge */}
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 w-full">
                      ...
                      {/* লাইভ কাউন্টডাউন টাইমার প্লেসমেন্ট */}
                      <CountdownTimer departureTime={booking.departureDateTime} status={booking.status} />
                    </div>

                    {/* Row 2: Journey Vector Timeline Grid View (Departure -> Line -> Destination) */}
                    <div className="flex items-center justify-between relative max-w-xl w-full">
                      ...
                      {/* Connecting Dashed Line Grid Vector */}
                    ...
                    {/* Row 3: Card Footer Aggregate Pricing & Payment CTA actions */}
                    <div className="pt-4 border-t border-zinc-100 dark:border-zinc-800/60 flex flex-col sm:flex-row sm:items-center justify-between gap-4 w-full">
                      ...
                      {/* Conditional Call to Actions UI Controls */}
                      <div className="sm:text-right shrink-0">
                            ...
                            <Button
                              ...
                              className={`h-11 px-6 text-xs font-black uppercase tracking-wider text-white transition-all rounded-xl shadow-md ${
                                isPaymentDisabled
                                  ...
                                  : "bg-black hover:bg-[#039855] dark:bg-zinc-100 dark:text-black dark:hover:bg-[#039855] dark:hover:text-white shadow-green-500/10"
                              }`}
                            >
                        ...
                        {booking.status === 'pending' && (
                          <div className="h-10 px-4 bg-amber-500/5 border border-amber-500/20 text-amber-600 dark:text-amber-500 rounded-xl flex items-center justify-center text-xs font-black uppercase tracking-wider gap-1.5">
                            <FiClock size={13} /> Awaiting Verification
                          </div>
                        )}
