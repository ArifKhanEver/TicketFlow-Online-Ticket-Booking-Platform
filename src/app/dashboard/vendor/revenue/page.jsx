...
export default function RevenueOverviewPage() {
  // ১. ডাইনামিক এপিআই সিঙ্কের জন্য রেডি করা মক স্টেট ডাটা স্ট্রাকচার
  const [revenueMetrics, setRevenueMetrics] = useState({
    totalTicketsAdded: 148,
    totalTicketsSold: 96,
    totalRevenueBDT: 135400,
    // চার্টের জন্য মাস ভিত্তিক ডেটা বিন্যাস (Recharts Matrix)
    monthlyAnalytics: [
      { name: 'Jan', added: 20, sold: 12, revenue: 15400 },
      { name: 'Feb', added: 25, sold: 18, revenue: 24000 },
      { name: 'Mar', added: 18, sold: 10, revenue: 12500 },
      { name: 'Apr', added: 35, sold: 24, revenue: 38000 },
      { name: 'May', added: 30, sold: 20, revenue: 29500 },
      { name: 'Jun', added: 20, sold: 12, revenue: 16000 },
    ]
  });

  // থিম ইন্টিগ্রেশনের জন্য কাস্টম চার্ট টুলটিপ ওভারলে কম্পোনেন্ট
  ...
  return (
    <motion.div 
      ...
      className="w-full max-w-6xl mx-auto px-4 md:px-6 space-y-8 py-4"
    >
      ...
      {/* THREE INTERACTIVE STATS CARDS (Total Added, Total Sold, Total Revenue) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 w-full">
        ...
        <Card className="bg-white dark:bg-[#111113] border border-zinc-200 dark:border-zinc-800/80 p-6 rounded-2xl flex flex-row items-center justify-between shadow-sm relative overflow-hidden group">
          <div className="space-y-1.5 relative z-10">
            ...
            <p className="text-3xl font-black text-zinc-900 dark:text-white tracking-tight">{revenueMetrics.totalTicketsAdded}</p>
        ...
        <Card className="bg-white dark:bg-[#111113] border border-zinc-200 dark:border-zinc-800/80 p-6 rounded-2xl flex flex-row items-center justify-between shadow-sm relative overflow-hidden group">
          <div className="space-y-1.5 relative z-10">
            <p className="text-xs font-bold uppercase tracking-wider text-zinc-400">Total Tickets Sold</p>
            <p className="text-3xl font-black text-[#039855] tracking-tight">{revenueMetrics.totalTicketsSold}</p>
        ...
        <Card className="bg-white dark:bg-[#111113] border border-zinc-200 dark:border-zinc-800/80 p-6 rounded-2xl flex flex-row items-center justify-between shadow-sm relative overflow-hidden group sm:col-span-2 lg:col-span-1">
          <div className="space-y-1.5 relative z-10">
            ...
            <p className="text-3xl font-black text-[#F05A28] tracking-tight">
              {revenueMetrics.totalRevenueBDT.toLocaleString()} <span className="text-xs font-bold text-zinc-400">BDT</span>
            </p>
      ...
      {/* DYNAMIC RECHARTS VISUALIZATION PLATFORM WINDOW */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 w-full">
        
        {/* Chart 1: Revenue Generation Trend (AreaChart) */}
        <Card className="bg-white dark:bg-[#111113] border border-zinc-200 dark:border-zinc-800/80 p-5 rounded-[24px] shadow-sm lg:col-span-7 space-y-4">
          ...
          <div className="w-full h-72">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={revenueMetrics.monthlyAnalytics} margin={{ top: 10, right: 5, left: -20, bottom: 0 }}>
                <defs>
        ...
        {/* Chart 2: Tickets Allocation vs Sales Volume (BarChart) */}
        <Card className="bg-white dark:bg-[#111113] border border-zinc-200 dark:border-zinc-800/80 p-5 rounded-[24px] shadow-sm lg:col-span-5 space-y-4">
          ...
          <div className="w-full h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={revenueMetrics.monthlyAnalytics} margin={{ top: 10, right: 5, left: -25, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e4e4e7" className="dark:stroke-zinc-800/50" />
                <XAxis dataKey="name" stroke="#a1a1aa" fontSize={11} tickLine={false} axisLine={false} />
                <YAxis stroke="#a1a1aa" fontSize={11} tickLine={false} axisLine={false} />
